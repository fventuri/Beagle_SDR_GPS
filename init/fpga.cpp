...
void fpga_init() {
    ...
    if (!raspsdr)
        ...
        fp = fopen(stprintf("%sRaspSDR.%s.bit", background_mode? "/usr/local/bin/":"", fpga_file) , "rb");
	...
	const char *aout = background_mode? "/usr/local/bin/kiwid.aout" : (BUILD_DIR "/kiwi.aout");
    fp = fopen(aout, "rb");
